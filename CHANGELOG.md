...
- Autogenerated OpenAPI examples now land on the JSON Schema `examples` list
  instead of the OAS `example` keyword when `openapi_version` is `3.2.0`
  or newer, because OpenAPI 3.2 deprecates `example`
  inside Schema Objects. Older versions keep writing `example`,
  and examples that you write by hand are never rewritten, #1485
- `XML.attribute` and `XML.wrapped` are now `bool | None` and default
  ...
  Both are deprecated in OpenAPI 3.2 in favour of `XML.node_type`,
  and the spec forbids using them together with it, #1485
...
- `dmr.openapi.mappers.schema_loader.load_schema` does not take
  ...
  Which keyword a generated example lands on depends on the target
  OpenAPI version, which that function does not know about, #1485
...
- Added the missing OpenAPI 3.2 fields to our spec objects, #1485:
  - `OpenAPIConfig.self_uri` and `OpenAPI.self_uri` for `$self`,
    setting it for OpenAPI older than `3.2.0` raises a `ValueError`
  - `Server.name`
  - `Tag.summary`, `Tag.parent`, and `Tag.kind`
  - `'querystring'` as a `Parameter.param_in` value,
    such parameters must use `content`, not `schema`

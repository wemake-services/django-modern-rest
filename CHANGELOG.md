...
- `MediaType` does not validate itself in `__post_init__` anymore,
  so an empty `MediaType()` or one that mixes `encoding`
  with `item_encoding` / `prefix_encoding` no longer raises `ValueError`.
  `openapi-spec-validator` reports these, we don't reimplement it, #1485

...
#: Callback that can rewrite a generated OpenAPI ``operationId``.
#:
#: Receives the library's default operation id and must return the final
#: value that will be written into the spec. Explicit ``operation_id``
#: values set on endpoints are not passed through this callback.
OperationIdCallback: TypeAlias = Callable[[str], str]
...
class OpenAPIConfig:
    ...
    Attributes:
        ...
        webhooks: Webhook definitions that may be initiated by the API,
            keyed by name.
        operation_id_callback: Optional callback used to customize
            generated ``operationId`` values. The default generator
            still runs first; the callback only rewrites that result.
            Explicit endpoint ``operation_id`` values are kept as-is.
    ...
    operation_id_callback: OperationIdCallback | None = None

...
def test_single_view_sync405(
    ...
    assert json.loads(response.content) == snapshot({
        'detail': (
            "Method 'delete' is not allowed, allowed: ['options', 'post']"
        ),
    })
...
def test_composed_view_sync405(dmr_client: DMRClient) -> None:
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert json.loads(response.content) == snapshot({
        'detail': "Method 'put' is not allowed, allowed: ['options', 'post']",
    })
...
def test_composed_view_async405(dmr_client: DMRClient, faker: Faker) -> None:
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert json.loads(response.content) == snapshot({
        'detail': "Method 'delete' is not allowed, allowed: ['options', 'put']",
    })

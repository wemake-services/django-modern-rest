...
def test_dynamic_throttle_before_negotiation_sync(
    dmr_rf: DMRRequestFactory,
    freezer: FrozenDateTimeFactory,
) -> None:
    """Ensure DynamicThrottle runs before response negotiation (sync)."""
    # This will fail with an 406 error:
    request = dmr_rf.get('/whatever/', headers={'Accept': 'wrong'})
    ...
    assert (
        request_endpoint(request, strict=True)
        is _DynamicSyncController.api_endpoints['GET']
    )
...
async def test_dyn_throttle_pre_negotiate_async(
    ...
    """Ensure DynamicThrottle runs before response negotiation (async)."""
    # This will fail with an 406 error:
    request = dmr_async_rf.get('/whatever/', headers={'Accept': 'wrong'})
    ...
    assert (
        request_endpoint(request, strict=True)
        is _DynamicAsyncController.api_endpoints['GET']
    )

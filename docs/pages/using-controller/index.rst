...
The simplest way to create an endpoint is to define a sync
...
1. It will know that the ``post`` endpoint will handle the ``POST`` HTTP method,
...
3. It will infer the default status code for ``post``, which will be ``201`` (*CREATED*).
   All other endpoints have ``200`` (*OK*) as the default
...
5. The same metadata will be used to render the OpenAPI spec

``django-modern-rest`` never creates implicit methods for you.
No ``HEAD``, no :ref:`OPTIONS <meta>`.
Create them explicitly if you need them.
...
or can use the :func:`~dmr.endpoint.modify` decorator
...
Other response specs can be specified via the
...
.. important::
  ...
  In case you don't have a controller / endpoint instance
  (like in a middleware, for example),
  you can fallback to using the :func:`~dmr.response.build_response`
...
.. note::
  ...
  This makes it more configurable.

"Real endpoints" can use the :func:`~dmr.endpoint.validate`
decorator, the :attr:`~dmr.controller.Controller.responses`
``Controller`` attribute,
...
The ``@validate`` decorator is useful but is not required
for real endpoint declarations.
Instead, you can specify response specs in the ``response`` field
...
If :ref:`response validation <response_validation>` passes, then all is fine!
...
Here's the top-level view on how the request / response lifecycle looks like:
...
- :attr:`~dmr.endpoint.Endpoint.serializer_context_cls`
  to customize how the model for serialization of incoming data is created
- :attr:`~dmr.endpoint.Endpoint.request_negotiator_cls`
  and
  :attr:`~dmr.endpoint.Endpoint.response_negotiator_cls`
  to customize how :doc:`content negotiation <../negotiation>` works for this endpoint

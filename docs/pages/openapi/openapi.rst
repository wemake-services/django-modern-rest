...
Set ``openapi_version='3.2.0'`` on :class:`dmr.openapi.OpenAPIConfig`
to target `OpenAPI 3.2 <https://spec.openapis.org/oas/v3.2.0.html>`_.

Everything that 3.2 adds to the spec objects is available:

- Self-identifying documents:
  ``OpenAPIConfig.self_uri``, dumped as ``$self``
- Named servers: :class:`~dmr.openapi.objects.Server` ``name``
- Tag hierarchy and classification:
  :class:`~dmr.openapi.objects.Tag` ``summary``, ``parent``, and ``kind``
- Whole-query-string parameters:
  :class:`~dmr.openapi.objects.Parameter` with ``param_in='querystring'``
- Reusable media types:
  :class:`~dmr.openapi.objects.Components` ``media_types``
- Response short label: :class:`~dmr.openapi.objects.Response` ``summary``
  and :class:`~dmr.metadata.ResponseSpec` ``summary``
- Media type prose and sequential media types:
  :class:`~dmr.openapi.objects.MediaType` ``description``
  and ``item_schema``
- Nested and sequential encoding:
  :class:`~dmr.openapi.objects.Encoding` ``encoding``,
  ``item_encoding``, and ``prefix_encoding``
- Structured and serialized examples:
  :class:`~dmr.openapi.objects.Example` ``data_value``
  and ``serialized_value``
- XML node typing: :class:`~dmr.openapi.objects.XML` ``node_type``,
  it deprecates ``attribute`` and ``wrapped``
- Discriminator fallback:
  :class:`~dmr.openapi.objects.Discriminator` ``default_mapping``
- OAuth 2.0 device grant:
  :class:`~dmr.openapi.objects.OAuthFlows` ``device_authorization``
  and :class:`~dmr.openapi.objects.OAuthFlow` ``device_authorization_url``
- OAuth metadata discovery and deprecating a scheme:
  :class:`~dmr.openapi.objects.SecurityScheme` ``oauth2_metadata_url``
  and ``deprecated``

Here's how several of them look together:

.. literalinclude:: /examples/openapi/openapi_v32.py
  :caption: urls.py
  :language: python
  :linenos:

Some of these fields do not exist in older versions of the spec.
We take care of that for you:

- ``self_uri`` raises a ``ValueError``
  when ``openapi_version`` is older than ``'3.2.0'``
- :attr:`ResponseSpec.summary <dmr.metadata.ResponseSpec>` is only rendered
  for ``'3.2.0'`` and newer, older versions silently drop it
- Autogenerated examples land on the JSON Schema ``examples`` keyword
  for ``'3.2.0'`` and newer, because 3.2 deprecates the OAS ``example``
  keyword inside Schema Objects.
  See :ref:`openapi-examples-generation`

.. versionadded:: 0.16.0
  Support for the OpenAPI 3.2 spec objects.
...
.. note::
  ...
  Which keyword a generated example lands on depends
  on your ``openapi_version``. OpenAPI 3.2 deprecates the OAS ``example``
  keyword inside Schema Objects in favour of the JSON Schema ``examples``
  list, so we write ``examples: [...]`` for ``'3.2.0'`` and newer
  and ``example: ...`` for older versions.

  Examples that you write by hand are never rewritten.

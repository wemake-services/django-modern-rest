...
each :class:`dmr.security.SyncAuth` and :class:`dmr.security.AsyncAuth`
...
:class:`~dmr.openapi.objects.SecurityRequirement` objects.

.. warning::

  ``security`` only documents a mechanism in the schema,
  it is never enforced at runtime. Use ``auth`` to actually
  protect your endpoints, see :doc:`/pages/auth/common`.

.. literalinclude:: /examples/openapi/security_requirements.py
  :caption: urls.py
  :language: python
  :linenos:
...
3. On an endpoint with ``security`` of
   :deco:`~dmr.endpoint.modify` and :deco:`~dmr.endpoint.validate`
...
.. note::
  ...
  or :meth:`dmr.security.SyncAuth.security_requirement` instead.
